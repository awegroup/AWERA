...
class PowerCurveConstructor:
    ...
    def run_optimization(self, wind_speed, power_optimizer, x0,
        ...
        for i in range(n_x0):
            ...
            try:
                # TODO log? print("Testing the {}th starting values: {}".format(i,
                #                                                    x0_test))
        ...
        if sum(sim_successfuls) > 0:
            ...
            # TODO log? print('All simulations failed, save flagged loose error simulation output')
        ...
        for i, vw in enumerate(self.wind_speeds):
            ...
            try:
                ...
                try:  # Retry for a slightly different wind speed.
                    # TODO log? print('first optimization/simulation ended in error: {}'.format(e))
                    ...
                    # TODO log? print("Optimization sequence stopped prematurely due to failed optimization. {:.2f} m/s is the "
            ...
            if sim_successful:
                x_opt_last = x_opt
                vw_last = vw
            # TODO log? print(self.wind_speeds[:i+1], [kpi['sim_successful'] for kpi in self.performance_indicators])
            # TODO log? print(len(self.wind_speeds[:i+1]), sum([kpi['sim_successful'] for kpi in self.performance_indicators]))
    ...
    def plot_optimal_trajectories(self, wind_speed_ids=None, ax=None, circle_radius=200, elevation_line=25*np.pi/180,
        ...
        while True:
            ...
            masking_counter += 1
        # TODO drop? print('Total Masking {} power runs'.format(masking_counter))
    ...
    def plot_optimization_results(self, opt_variable_labels=None, opt_variable_bounds=None, tether_force_limits=None,
        ...
        if not self.plots_interactive:
            plt.savefig(self.plot_output_file.format(title='optimization_results{}'.format(plot_info)))
