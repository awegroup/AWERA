...
def grid_to_grid_bilinear(data_points, level_dependent=True, time_dependent=True):
    if level_dependent:
...
def histos_single_location(location_lat, location_lon, start_year, final_year, do_bilin_interp=False):
    ...
    if do_bilin_interp:
        ...
        surface_pressure = grid_to_grid_bilinear(surface_pressure_5, level_dependent=False)

        surf_elev = grid_to_grid_bilinear(surf_elev_5, level_dependent=False, time_dependent=False)
        print(surf_elev)
    else:
    ...
    return array_level_heights, array_level_height_diffs, ceil_velocities_500m, height_velocities_100m, levels,\
        surface_pressure, q_levels[:, list(levels).index(114)], q_levels[:, list(levels).index(129)],\
        t_levels[:, list(levels).index(114)], t_levels[:, list(levels).index(129)],\
        v_levels[:, list(levels).index(114)], v_levels[:, list(levels).index(129)],\
        v_levels_east[:, list(levels).index(114)], v_levels_east[:, list(levels).index(129)],\
        v_levels_north[:, list(levels).index(114)], v_levels_north[:, list(levels).index(129)]


def histos_single_location_100m_fixed_height_data(location_lat, location_lon, start_year, final_year):
    """"Execute analyses on the data of single grid point.
    ...
    """
    if not start_year == final_year:
        print('multiple years not yet supported')
        sys.exit()
    year = start_year
    file_name = '/cephfs/user/s6lathim/ERA5Data-fixed-height/fixed_height_wind_speeds_{}.nc'.format(year)
    ...
    v_levels_east = ds['u100'][:, i_lat, i_lon].values
    v_levels_north = ds['v100'][:, i_lat, i_lon].values
    v_levels = (v_levels_east**2 + v_levels_north**2)**.5
...
def make_basic_data_plots(level, location_info, t, q, v, v_comp, u_comp):
    plot_histogram(level, list(t.flat), hist_type='temperature',
                   mean_label=r'Average temperature',
                   x_label=r'temperature in K',
                   title=r'Temperature distribution for model level {}'.format(level),
                   info=location_info, ml=False)
    plot_histogram(level, list(q.flat), hist_type='humidity',
                   mean_label=r'Average humidity',
                   x_label=r'humidity in kg/kg',
                   title=r'Humidity distribution for model level {}'.format(level),
                   info=location_info, ml=False)
    plot_histogram(level, list(v.flat), hist_type='absolute_velocity',
                   mean_label=r'Average velocity',
                   x_label=r'wind speed in m/s',
                   title=r'Wind speed distribution for model level {}'.format(level),
                   info=location_info, ml=False)
    plot_histogram(level, list(v_comp.flat), hist_type='velocity_north',
                   mean_label=r'Average velocity',
                   x_label=r'wind speed in m/s',
                   title=r'Northern wind speed distribution for model level {}'.format(level),
                   info=location_info, ml=False)
    plot_histogram(level, list(u_comp.flat), hist_type='velocity_east',
                   mean_label=r'Average velocity',
                   x_label=r'wind speed in m/s',
                   title=r'Easter wind speed distribution for model level {}'.format(level),
                   info=location_info, ml=False)
...
def make_single_location_histograms(additional_locations=[], do_bilin_interp=False):
    ...
    for loc in locations:
        ...
        array_level_heights, array_level_height_diffs, ceil_velocities_500m, height_velocities_100m, levels,\
            surface_pressure, q_114, q_129, t_114, t_129, v_114, v_129, u_comp_114, u_comp_129, v_comp_114,\
            v_comp_129 = histos_single_location(location_lat, location_lon, start_year, final_year,
                                                do_bilin_interp=do_bilin_interp)

        # Plot surface pressure histogram
        plot_histogram(0, list(surface_pressure.flat), hist_type='surface_pressure',
                       mean_label=r'Average surface pressure',
                       x_label=r'surface pressure in Pa',
                       title=r'Surface pressure distribution',
                       info=location_info, ml=False)
        # Plot data on humidity, temperature and wind speeds for chosen model levels
        level = 114
        make_basic_data_plots(level, location_info, t_114, q_114, v_114, v_comp_114, u_comp_114)
        level = 129
        make_basic_data_plots(level, location_info, t_129, q_129, v_129, v_comp_129, u_comp_129)
...
def make_single_location_histograms_fixed_height_data(additional_locations=[], do_bilin_interp=False):
    # Single location histograms:
    print('Make single location height and velocity histograms')
    # Define locations
    locations = [(50, -20),  # <0m surface Atlantic Ocean Point Fiona
    ...
    for loc in locations:
        ...
        location_lat = loc[0]
        location_lon = loc[1]
        location_info = '_lat_' + str(location_lat) + '_lon_' + str(location_lon) + '_fixed_100m_data'
        if do_bilin_interp:
            location_info = location_info + 'bilin_interp'

        v_100m = histos_single_location_100m_fixed_height_data(location_lat, location_lon, start_year, final_year)

        # Velocity for 100m ceiling
        plot_histogram(0, list(v_100m.flat), hist_type='velocity_100m_fixed',
                       mean_label=r'Average wind speed',
                       x_label=r'wind speed in m/s',
                       title=r'Maximal wind speeds to be reached for 100m ERA5 height',
                       info=location_info, ml=False)


if __name__ == '__main__':
    print("processing monthly ERA5 data from {:d} to {:d}".format(start_year, final_year))
    # Single location plots for 100m height data from ERA5
    # make_single_location_histograms_fixed_height_data(do_bilin_interp=False)
    # sys.exit()
    # Single location histograms:
    make_single_location_histograms(do_bilin_interp=False)
    sys.exit()
